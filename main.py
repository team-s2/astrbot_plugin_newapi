    ...
    class NewApiPlugin(star.Star):
        ...
        async def _list_channels(self, event: AstrMessageEvent):
            ...
            try:
                ...
                for ch in shown:
                    ...
                    status = CHANNEL_STATUSES.get(
                        int(ch.get("status", 0)), "未知"
                    )
                    rows.append([
                        f"#{ch.get('id')}",
                ...
                if total > limit:
                    lines.append("")
                    lines.append(
                        "可用 /newapi channel <名称或 ID> 查看具体渠道。"
                    )
        ...
        async def _show_channel(self, event: AstrMessageEvent, query: str):
            ...
            try:
                ...
                models = [
                    item
                    for item in str(found.get("models") or "").split(",")
                    if item
                ]
                lines = [
        ...
        async def flow(self, event: AstrMessageEvent):
            ...
            try:
                ...
                rows = await self.client.flow(
                    end_timestamp - hours * 3600, end_timestamp
                )
                if not rows:
